...
def main() -> int:
    ...
    ultimo = max((i[:10] for i in ocupados if i[:4].isdigit()), default=None)
    dia = datetime.date.today() + datetime.timedelta(days=1)
    if ultimo:
        dia = max(dia, datetime.date.fromisoformat(ultimo) + datetime.timedelta(days=1))
    libres: list[str] = []
    ...
    print("FRANJAS LIBRES (toma las tres primeras, una por franja):")
