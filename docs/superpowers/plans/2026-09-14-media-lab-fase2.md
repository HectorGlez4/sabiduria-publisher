...
9. **Guardia de envío fuera de `compartir` (revisiones C1 e I-5; corregida en `95a421c` tras una revisión de especificación de la tarea 3, B1-B3 e I1-I3).** Cuenta como envío el texto exacto de `textos.ENVIO` y `ENVIO_HISTORIA` (publicación directa de Stories en francés, español e inglés), una etiqueta de `ENVIO_HISTORIA` seguida de un separador no alfanumérico («Votre story, 2 nouvelles», «Amis proches (12)»: I1), el id del botón de publicar de Threads y cualquier etiqueta que empiece por uno de `PREFIJOS_ENVIO`, salvo las excepciones explícitas `NO_ENVIO` (conmutadores y navegación que S1 confirma), que solo exceptúan de la regla de prefijos y nunca anulan un texto exacto (o por prefijo de `ENVIO_HISTORIA`) de `ENVIO` o `ENVIO_HISTORIA`. La comparación pasa por `textos.normalizar` (I2: NFKC, sin caracteres de categoría `Cf` —incluidos los de ancho cero—, espacios colapsados y casefold), para que un NBSP, un espacio fino o una marca invisible no la esquiven. Los prefijos en inglés («post», «share», «send») también atrapan etiquetas como «Posts» o «Shared»: la guardia falla cerrado (no toca); si eso bloquea un flujo, se añade una excepción confirmada, no se quita el prefijo. `pantalla.es_envio(xml, n, paquete, ignorar=())` cuenta como envío tanto el propio nodo `n` como cualquier nodo PULSABLE (de cualquier paquete del volcado, no solo `paquete`: I3, falla cerrado) cuyas bounds contengan el centro del toque, si él mismo o cualquiera de sus descendientes es de envío (B1: cubre tanto el antecesor clickable de la propia rama como cualquier otro contenedor pulsable de otra rama que solape ese punto, no solo un envío que toque el centro directamente). `pasos.tocar(n, xml, paquete, permitir=())` no pulsa ningún control de envío y es la vía de los toques sobre controles localizados en el volcado de los flujos nuevos. **No** pasan por ella, porque no apuntan a un control de la app sino a un campo de texto o a una posición calculada: el toque del campo en `escribir_texto` y `pegar_en`, el «+» del chip de audio (`punto_mas`), `tocar_derecha` en el visor y los arrastres; el feed de Instagram de la fase 1 (`instagram_feed`) conserva sus toques directos salvo la selección múltiple. `permitir` es una lista blanca explícita por CAMPO (B2): solo deja de contar el campo (texto, desc o resource-id) cuyo valor exacto coincide con una etiqueta de `permitir` en ESE nodo, nunca el nodo entero (un `desc` permitido no oculta un `texto` o un id de envío del mismo nodo, ni ningún otro control de envío en ese punto); solo la usa `_visor_propio`: el avatar «Votre story» del perfil de Instagram (con el perfil de la marca verificado) y la miniatura «Tu historia» del inicio de Facebook (con el inicio y la Página verificados), que abren el visor y no publican. La spec no preveía esta guardia.
...
Añadir a la tupla `PATRONES["instagram"]` el formato de la content-desc de las miniaturas del selector de Story que anotó S1, para que la poda no la vacíe:
...
def seccion_instagram_historia() -> None:
    ...
    chip = IP.chip_audio_historia(fixture("instagram", "historia-editor-audio"))
    check(chip is not None and bool(chip["tema"]), f"A: el chip de audio del editor real dice el tema ({chip})")
...
```python
    ...
    editor_audio = fixture("instagram", "historia-editor-audio")
...
Añadir a la tupla `PATRONES["threads"]` el formato de la content-desc de las miniaturas de la galería que anotó S1, para que la poda no la vacíe:
...
Sustituir `DESCARTE["facebook"]` por `{"titulos": ("¿Descartar publicación?", "¿Guardar borrador?"), "botones": ("Descartar",)}` con los textos observados, y `PATRONES["facebook"]` por:
...
Con lo anotado en S3: sustituir en `textos.py` los valores de `seleccion_multiple`, `publicacion_cuadricula` y `seleccionar_varios` por los observados y quitar su marca; añadir a las tuplas `PATRONES` de las tres apps el patrón `r"\d{1,2}"` (los ordinales de la selección múltiple). Si S3 mostró el ordinal en la content-desc y no como texto hijo, adaptar `pantalla.ordinal_de` (tarea 17a, Step 3) para leerlo de ahí y ajustar `xml_rejilla` en la prueba. Volver a podar los cinco volcados de S3 con `lab.py fixture-podar` a los fixtures de **Files** y abrirlos con Read.
...
- `textos`: `ENVIO`, `ENVIO_IDS`, `ENVIO_HISTORIA`, `PREFIJOS_ENVIO`, `NO_ENVIO`, `NO_TOCAR`, `DESCARTE`, `TITULOS_BORRADO`, `EDADES`, `PATRONES`, `texto`, `es_texto_envio`, `conocidos`, `permitido`.
- `pantalla`: `elegir`, `coincidencias`, `nodo`, `por_id`, `pulsable`, `tiene_texto`, `hay_desplegable`, `evaluar_envio`, `se_solapan_verticalmente`, `emergente_solapada`, `parece_desplegable`, `describe_emergente`, `pista_idioma`, `dentro`, `es_envio(xml, n, paquete, ignorar)`, `nodo_sonda`, `boton_descarte`, `miniatura_por_hora`, `edad_segundos`, `marcado_en_fila`, `estado_confirmado`, `imagen_bajo`, `tema_bajo_cabecera`, `aviso_corto`, `observacion_envio`, `campos`, `miniaturas`, `ordinal_de`, `selecciones_ordenadas`.
- `pasos`: `exigir_listo`, `esperar_que`, `volcado_fresco`, `esperar_estable`, `escribir_texto(texto, paquete, *, campo, exigir, desplegable_nodos, emergente)`, `atras`, `descartar`, `tocar(n, xml, paquete, permitir)`, `observar_envio`, `enviar(..., listo(xml, emergentes), ...)`, `lanzar_app`, `colocar_sticker`, `pegar_en`, `encuesta_de_historia`, `seleccionar_en_orden(paquete, subidas, lector_fecha, boton_multiple, xml_boton)`, `recorrer_fotogramas`, `deslizar_media`, `tocar_derecha`.
