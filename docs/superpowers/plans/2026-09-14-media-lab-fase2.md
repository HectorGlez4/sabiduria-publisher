...
9. **Guardia de envío fuera de `compartir` (revisiones C1 e I-5).** Cuenta como envío el texto exacto de `textos.ENVIO` y `ENVIO_HISTORIA` (publicación directa de Stories en francés, español e inglés), el id del botón de publicar de Threads y cualquier etiqueta que empiece por uno de `PREFIJOS_ENVIO`, salvo las excepciones explícitas `NO_ENVIO` (conmutadores y navegación que S1 confirma), que solo exceptúan de la regla de prefijos y nunca anulan un texto exacto de `ENVIO` o `ENVIO_HISTORIA`. Los prefijos en inglés («post», «share», «send») también atrapan etiquetas como «Posts» o «Shared»: la guardia falla cerrado (no toca); si eso bloquea un flujo, se añade una excepción confirmada, no se quita el prefijo. `pasos.tocar(n, xml, paquete, permitir=())` no pulsa ningún control de envío y es la vía de los toques sobre controles localizados en el volcado de los flujos nuevos. **No** pasan por ella, porque no apuntan a un control de la app sino a un campo de texto o a una posición calculada: el toque del campo en `escribir_texto` y `pegar_en`, el «+» del chip de audio (`punto_mas`), `tocar_derecha` en el visor y los arrastres; el feed de Instagram de la fase 1 (`instagram_feed`) conserva sus toques directos salvo la selección múltiple. `permitir` es una lista blanca explícita que solo deja de contar los nodos con esa etiqueta (cualquier otro control de envío en ese punto sigue bloqueando) y solo la usa `_visor_propio`: el avatar «Votre story» del perfil de Instagram (con el perfil de la marca verificado) y la miniatura «Tu historia» del inicio de Facebook (con el inicio y la Página verificados), que abren el visor y no publican. La spec no preveía esta guardia.
