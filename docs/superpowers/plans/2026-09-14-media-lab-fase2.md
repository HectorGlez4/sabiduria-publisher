...
32. **Recuento del perfil refrescado antes de leer `publicaciones_antes` y `publicaciones_despues`.** Medido en el teléfono el 2026-09-15 (`findings.md`): con el perfil en memoria, Instagram mostró 3718 cuando ya eran 3719, y `ig compartir` habría dado `conteo_no_cuadra` con una publicación buena. `instagram_feed._recuento_fresco` desliza UNA vez hacia abajo (`telefono.deslizar`, nuevo: `input swipe`, del estilo de `tocar`) con el gesto de `instagram_pantallas.gesto_de_refresco` (x en el centro de «Modifier le profil»; y desde el punto medio entre el recuento y el botón, 600 px hacia abajo sin salir del volcado), espera 3 s y exige el mismo recuento en `pasos.VOLCADOS_LIMPIOS` (3) volcados frescos con `pasos.esperar_estable`. Si el inicio o el final del gesto caen sobre algo que no se toca (`pantalla.punto_bloqueado`: las reglas de `nodo_sonda` para envío, id de envío y borrado) no desliza y se queda la lectura sin refrescar, con aviso; si el gesto o la espera fallan, el recuento es None, con aviso. `ig abrir` añade `recuento_refrescado` y `avisos`, y tras un gesto que no estabiliza vuelve a esperar el perfil de la marca antes de tocar «Créer». En `compartir` todo es posterior a pulsar: cualquier excepción del refresco va a `avisos`. Sección 19 de las pruebas; las de `abrir` de las secciones 6 y 18 llevan tres volcados más del perfil en su guion. **Revisión de seguridad:** (I1) `_recuento_fresco` relanza `TelefonoNoListo` (hereda de `PantallaInesperada`; convertido en aviso, `abrir` tocaba «Créer» igual) y vuelve a exigir el teléfono listo tras estabilizar: `ig abrir` sale con 4 sin tocar nada más; en `compartir` el `except Exception` de `despues` lo deja en aviso y `confirmado_sin_conteo`. (I2) `--publicaciones-antes` sigue siendo obligatorio en `ig compartir`, pero acepta `none`/`null` (sin distinguir mayúsculas) como None, con un centinela como valor por defecto; `abc` sale con 2 y con None `compartir` da `confirmado_sin_conteo` (código 5). (Menores) `DESLIZAR_MIN_PX = 300`: un gesto más corto tras recortar al alto del volcado es `PantallaInesperada` y no se desliza. Antes de deslizar, `pantalla.motivo_tapado` exige que lo que se dibuja encima en el inicio sea de Instagram (ningún nodo de otro paquete después del último de Instagram que lo contiene). Es la regla de `telefono.tapado` llevada a un punto: `tapado` tal cual da siempre tapado con un nodo sintético y también con el nodo más profundo, porque en los volcados reales `modal_container` y `overlay_layout_container` van después. Además se exige que ninguna emergente se solape con el inicio (`emergente_solapada`, como `_tocar_sonda`); si no se pueden leer, no se desliza. Un volcado ilegible (`TelefonoError`) al situar el gesto también es aviso. Estabilidad con `VOLCADOS_LIMPIOS`. **Re-revisión:** (1) `instagram_pantallas.hoja_abierta` (pura) evita el gesto, con aviso, si hay una hoja o un modal de Instagram abierto: un nodo de Instagram con resource-id `background_dimmer`, `layout_container_bottom_sheet` o `bottom_sheet_container`, o `modal_container`/`overlay_layout_container` con descendientes. Son los ids medidos en `ig-create.xml` (hoja «Créer» abierta) y `ig-profile.xml` (en reposo, contenedores vacíos); las pruebas usan XML sintético con esos ids y esa estructura, sin textos de los volcados. `abrir` sigue y toca «Créer». (2) `_recuento_fresco` devuelve `bloqueo_ajeno` cuando algo de otra app tapa el inicio (`motivo_tapado`) o una emergente de Instagram se solapa con él; esto se mira antes que `punto_bloqueado`. Con él, `abrir` sale con 4 (`PantallaInesperada`) sin tocar «Créer»; en `compartir` es solo aviso. Las pruebas de la sección 19 de la notificación de systemui y de la emergente solapada pasan de «abrir sigue» a «abrir sale con 4», y se añade un diálogo de `permissioncontroller`. (3) El docstring dice que una notificación flotante real no sale en el volcado ni en las emergentes y no se detecta. (4) Además de `TelefonoNoListo`, se propagan los errores de programación inesperados (falla cerrado). **Revisión de calidad:** (I-1) la primera prueba de refresco de la sección 19 mueve «Créer» en el perfil refrescado y exige el toque en el centro nuevo y no en el viejo: sin ella, tocar «Créer» sobre el volcado de antes del refresco no hacía fallar nada. (m-1) `_recuento_fresco` devuelve el `NamedTuple` `RecuentoFresco(recuento, refrescado, deslizado, xml, bloqueo_ajeno)`, y las comprobaciones previas al gesto pasan a `_motivo_para_no_deslizar(xml, gesto) -> (motivo, ajeno)`. (m-2) Sus dos `except` capturan lo mismo (`PantallaInesperada`, `TelefonoError`, `OSError`) y antes relanzan `TelefonoNoListo`. (m-4) Docstrings cruzados: `telefono.tapado` es para un nodo que se va a tocar (cuenta cualquier nodo posterior, también de la propia app); `pantalla.motivo_tapado`, para un punto (solo lo de otro paquete detrás del último nodo propio). (m-5) `hoja_abierta` compara el id completo `com.instagram.android:id/<nombre>`. (m-6) `--publicaciones-antes` solo acepta enteros ASCII no negativos, o `none`/`null`: «３７１９», «٣٧١٩», «3_719», «+5» y «-1» salen con 2. (m-7) `__all__` partido en líneas. (m-8) Docstring de `abrir`, `findings.md` y el `ver` de `ig compartir` en `recetas.py` (`--publicaciones-antes none` si `ig abrir` lo dio null). Falta comprobarlo en el teléfono real.
33. **Tarea 7: ajustes al código real.** La prueba es la sección 20 (18 y 19 ya eran el arranque en frío y el recuento refrescado). `cmd_preflight` no se sustituye entera: solo se añaden `versiones` (con `tel.get("adb")` y un `try` que nunca rompe) y `agente_phone_awake` a la salida, que conserva `ahora`, `telefono`, `github` y `espera` con el mismo manejo de errores. `telefono.versiones` usa `VERSIONES_TIMEOUT_S = 15` por app en vez de 30 (con las cuatro apps de `textos.PAQUETES` el peor caso baja de 120 s a 60 s en cada `preflight`), con el mismo contrato: None si esa app falla (medido en el teléfono el 2026-09-15: `preflight` entero en 3,65 s y 0-1 s por `dumpsys package`, así que 15 s se queda). La sección 20 añade a la del plan: `versiones` con `shell` parcheado (orden exacta, timeout y None por app), `adb`/`shell` prohibidos durante `preflight`, un fallo de `versiones` que sale como `{"error": …}` con código 0 y las claves de siempre, y `_agente_despierto` con `subprocess.run` parcheado (True con código 0, False con otro, None con `OSError`, `TimeoutExpired` o `SubprocessError`; solo `launchctl list <label>` con `stdin`, `stdout` y `stderr` a `DEVNULL` y timeout). Ninguna prueba llama a `launchctl` de verdad. **Revisión:** (I-1) `_agente_despierto` descarta la salida en vez de capturarla (no se usa y así no hay `UnicodeDecodeError` posible) y captura `(OSError, subprocess.SubprocessError)`; `cmd_preflight` envuelve además la llamada en un `try` que da `agente_phone_awake: None` ante cualquier `Exception`, porque un `RuntimeError` escapaba de `main` sin JSON y un `UnicodeDecodeError` salía con código 2 perdiendo `telefono`, `versiones`, `github` y `espera` en el paso 3 de la ventana. Pruebas: `RuntimeError` y `UnicodeDecodeError` desde `_agente_despierto` dan código 0, `agente_phone_awake` None y las claves de siempre. (m-1) Las llamadas directas a `telefono.versiones` y `_agente_despierto` de la sección 20 van en un `try` que convierte una excepción inesperada en `check(False, …)`: una mutación marca ✗ y no se lleva por delante las secciones siguientes.
34. **Tarea 8: el prompt parte del vigente (`30d3faf`), no del texto del plan**, para conservar las reglas posteriores: comandos sin componer, `displayTitle`, métricas, `SinVolcado`, `arranque_en_frio` y `none`. Sobre él se aplican solo los cambios de fase 2 del Step 3. La prueba es la sección 21 (la 19 ya existía): además de los fragmentos del plan exige los de esas reglas («nunca le añadas», «displayTitle», «media-lab-result-», «media-lab-verification-», «-M24H», «metricas-registrar», «SinVolcado», «arranque_en_frio», «`none`»…). Como la receta ya no nombra `ig abrir` en el prompt, `avisos` y `arranque_en_frio` pasan a regla general de cualquier paso de teléfono (7b), y un valor anotado null se pasa como `none`. El paso 5 cambia `TELEFONO_FASE_1` por `lab.py receta --celda <id>` (sale con 0) para teléfono y `API_FASE_1` para API. **Revisión:** (I1) tras un 5 el único comando de teléfono es `lab.py telefono-captura` para conciliar, nunca `telefono-atras`, `telefono-descartar` ni otro `ig`, porque la publicación pudo salir; la regla del borrador abierto vale solo tras un 4 en `preparar`, o en `publicar` antes de pulsar, o al abandonar una celda de teléfono por QA, y nunca tras un 5 (reglas de códigos y de borrador abierto, y 7e). (Menores) Solo `publicaciones_antes` puede ser null (se pasa como `--publicaciones-antes none` y sale `confirmado_sin_conteo`); otro valor anotado null es un 4. Los valores con espacios van entre comillas dobles y nunca quedan `<…>` ni corchetes en la línea (7b y 7e). 7f solo remite a la `verificacion` de la receta. La sección 21 comprueba esto sobre la regla o el paso concreto (texto entre su inicio y el siguiente), además de «2026-10-02» y `--sin-telefono`.
35. **Zona segura de Story en `render_overlay.py` (arriba ~250 px, abajo ~260 px; QA FAIL de CELL-012/014, 2026-09-15).** El overlay 9:16 queda entero en `ZONA_SEGURA_HISTORIA` = (50, 250, 1030, 1660): panel 250–680, píldoras de aviso y marca 1607–1660 (el aviso sube a 1542–1595 si no cabe junto a la marca) y subtítulo ajustado de 42 a 34 px. Si no cabe, `render` lanza `FueraDeZonaSegura` sin dibujar (`lab.py render` sale con 2) y el paso 7a del prompt prohíbe quitar el aviso o inventar texto: solo vale una versión más corta que dé el brief, o bloquear la celda. `cajas_story(...)` da las cajas; el feed 4:5 no cambia (hash fijado en la sección 22). `ZONAS_TEXTO_HISTORIA` de `stickers.py` (11b-1) debe salir de las constantes de `render_overlay` (panel y franja de píldoras), no de los literales antiguos `(54, 170, 1026, 600)` y `(0, 1775, 1080, 1828)`.
36. **Excepciones medidas en S1 para la regla del centro (`quick_capture_root_container`) y para `tapado` (`fast_scroll`).** La sonda S1 de Instagram (2026-09-15, Instagram 446.0.0.49.77, findings «Sonda S1 de la fase 2») falló cerrado en dos pantallas legítimas; la prueba es la sección 23, con XML sintético de los mismos ids, bounds y estructura. **A.** `pantalla.CONTENEDORES_RAIZ_IGNORADOS = {"com.instagram.android:id/quick_capture_root_container"}`: en `_bajo_el_toque` no se recorren los descendientes de ese contenedor solo si `contenedor_raiz_ignorado` (pulsable, resource-id completo igual, del paquete del id y ≥ 95 % de `ANCHO_PANTALLA_PX` y de `alto_volcado`) y además el centro cae dentro de otro pulsable descendiente suyo (`_pulsable_intermedio`, p. ej. `asset_button` alrededor de «Stickers»; defensa pedida por la revisión de seguridad): en el lienzo vacío, con un envío en pantalla, vuelve a bloquear. Siguen bloqueando `n` y el propio contenedor si tienen etiqueta de envío, cualquier nodo de envío o de borrado bajo el centro (pulsable o no) y los descendientes de cualquier otro pulsable bajo el centro (`asset_button` incluido). Vale para las tres guardias que pasan por `_bajo_el_toque` (envío, id de envío sin etiqueta, que no recorre descendientes, y borrado) y para `punto_bloqueado`. **B.** `telefono.NODOS_SUPERPUESTOS_IGNORADOS = {"com.instagram.android:id/fast_scroll"}`: en `tapado` ese nodo posterior no tapa solo si `superpuesto_ignorado` (resource-id completo igual, del paquete del id, no pulsable y sin texto ni content-desc, ni él ni sus descendientes); sus descendientes se miran por su cuenta y lo de otro paquete tapa siempre. `motivo_tapado` no cambia. **Hallazgo «Ami(e)s proches»:** el botón de mejores amigos del editor real dice «Ami(e)s proches», no «Amis proches», así que `es_texto_envio` no lo reconocía y solo lo bloqueaba la barra pulsable `story_share_controls_action_bar` que contiene «Vos stories» (con la excepción A, la única defensa); `textos.ENVIO_HISTORIA` añade «Ami(e)s proches» y «Ami(e)s proche» (I1 de la revisión). **«Stickers» sigue tapado en el editor real:** para `telefono.tapado` lo tapan un View hermano más ancho y `capture_interactive_drawable_container`, `camera_photo_texture_view` y otros contenedores de cámara de pantalla completa que van después en el documento, así que `sonda tocar --desc Stickers` sigue fallando cerrado (fijado en la sección 23). Por decisión de la revisión de seguridad no se afloja más: la Story de Instagram queda para una sesión con el usuario. Faltan la comprobación en el teléfono (que tocar la miniatura bajo `fast_scroll` la selecciona y no desplaza la rejilla) antes de hacer push o repetir S1.
...
- [x] **Step 3: Implementar en `telefono.py`**
...
- [x] **Step 4: Implementar en `lab.py`**
...
- [x] **Step 5: Comprobar la cola de 10 de la tarea 15**
...
MASTER = (1080, 1920)
ZONAS_TEXTO_HISTORIA = ((54, 250, 1026, 680), (50, 1542, 1030, 1660))  # panel y dos filas de píldoras de render_overlay (desviación 35); una prueba comprueba que coinciden con HISTORIA_PANEL y ZONA_SEGURA_HISTORIA
ARRASTRES_MAX = 2
