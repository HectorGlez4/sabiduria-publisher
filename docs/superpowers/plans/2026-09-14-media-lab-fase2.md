...
28. **Zona segura de Story en `render_overlay.py` (arriba ~250 px, abajo ~260 px; QA FAIL de CELL-012/014, 2026-09-15).** El overlay 9:16 queda entero en `ZONA_SEGURA_HISTORIA` = (50, 250, 1030, 1660): panel 250–680, píldoras de aviso y marca 1607–1660 (el aviso sube una fila si no cabe junto a la marca) y subtítulo ajustado de 42 a 34 px. Si no cabe, `render` lanza `FueraDeZonaSegura` sin dibujar. `cajas_story(...)` da las cajas; el feed 4:5 no cambia. `ZONAS_TEXTO_HISTORIA` de `stickers.py` (11b-1) debe salir de `render_overlay.cajas_story`, no de los literales antiguos `(54, 170, 1026, 600)` y `(0, 1775, 1080, 1828)`.
