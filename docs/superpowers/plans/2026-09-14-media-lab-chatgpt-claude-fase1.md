...
Ejecutar el prompt anterior a mano en esta sesión, paso a paso, con al menos un encargo aprobado para una celda `instagram/feed_single_image/android_native` de `coverage.json`. Si hace falta, antes crear el encargo y esperar a la automatización de Codex o usar `lab.py generar`. Confirmar con el usuario antes del paso 7e: es la primera publicación del flujo nuevo.
